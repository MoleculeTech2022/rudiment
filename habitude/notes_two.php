...
// Fetch the subject corresponding to the selected chapter
    $sql = "SELECT subject FROM i_upsc_notes WHERE chapter = '$chapter' LIMIT 1";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // Fetch the subject from the result
...
<head>
    ...
    <link href='https://unpkg.com/boxicons@2.1.1/css/boxicons.min.css' rel='stylesheet'>
     ...
     <title>HABITUDE WISE NOTES</title>
...
<?php
    ...
    <div class="add_section" style="margin-top: 90px; display: flex; align-items: center;">
        ...
        <span style="font-size: 20px; margin-left: 15px;display:flex;"><h4>Chapter</h4><?= " : " . $chapter; ?></span>
    ...
    <div class="container" id="notes_container" style="margin-top:15px;">
        <?php
        // Database connection
        include 'dbcon.php';
        
        // SQL query to fetch data from the database
        $sql = "SELECT note_id, `date`, `subject`, subtopic, dt FROM i_upsc_notes WHERE chapter = '$chapter' ORDER BY dt DESC";
        $result = $conn->query($sql);
