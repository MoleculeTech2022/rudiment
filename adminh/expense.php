...
<body>
    <div class="container">
        ...
        <div class="dashboard-contents"
            ...
            <!-- showing total expense details -->
            <div class="showing-exp-details"
                style="position:absolute;width:900px;height:50px;background-color:#fff;box-shadow:1px 1px 1px 1px #edeaea;border-radius:10px;margin-left:5px;margin-top:10px;">

                <div class="title" style="margin-left:30px;margin-top:10px;width:200px;">
                    <?php

                    //php script to fetch default acdyear
                    $sqlDefaultacdyear = "SELECT default_acdyear FROM dhacdyear";
                    $acdResult = mysqli_query($con, $sqlDefaultacdyear);
                    if (mysqli_num_rows($acdResult) > 0) {
                        while ($acdyear = mysqli_fetch_assoc($acdResult)) {
                            $defaultAcdYear = $acdyear['default_acdyear'];
                            // echo "this is very is code jdfhjdhfjfhdjfhdj " . $acdyear[]
                            ?>
                            <?php
                            //php script to sum of total expense
                            $sqlTotalExp = "SELECT sum(expAmt) AS totalExp FROM hexpense WHERE acdyear = '$defaultAcdYear' ";
                            $resultTotalExp = mysqli_query($con, $sqlTotalExp);
                            if (mysqli_num_rows($resultTotalExp) > 0) {
                                while ($rows = mysqli_fetch_assoc($resultTotalExp)) {
                                    ?>
                                    <?php echo "<h4 style='margin-top:0px;margin-left:0px;width:300px;'>Total Expense : " . $rows['totalExp'];
                                    "</h4>"; ?>

                                    <div style="margin-top:-23px;width:300px;margin-left:230px;">
                                        <?php
                                        echo "<span>Academic Year : " . $acdyear['default_acdyear'] . "</span>";
                                        ?>
                                    </div>
                                    <?php
                                }
                ...
                <div class="select-student-form">
                    <form action="code.php" method="POST">
                        ...
                        <select name="expCatgid" placeholder="Your Amount"
                            ...
                            <?php
                            $sqlCatgid = "SELECT expCatgid,expCatgTitle FROM hexpensecategory";
                        ...
                        <input type="number" name="expamt" id="expamt" placeholder="Expense Amount"
                            style="margin-left:30px;margin-top:20px;padding:5px;width:210px;border-radius:5px;border-width:1px;">

                        <select name="expmode" placeholder="Expense Mode"
                            style="margin-left:30px;margin-top:20px;padding:5px;width:210px;border-radius:5px;border-width:1px;">
                            <option value="Not Selected">Select Expense Mode</option>
                        ...
                        <input type="date" title="Expense Date" name="expdate" id="expdate" placeholder="Expense Date"
                            style="margin-left:30px;margin-top:20px;padding:5px;width:210px;border-radius:5px;border-width:1px;">
            ...
            <div>
                <table class="content-table" id="student-table" style="margin-left :30px;margin-top:-230px;">
                    ...
                    <tbody>
                        <?php
                        $count = 0;
                        $sqlExpense = "SELECT * FROM hexpense JOIN hexpensecategory ON hexpense.expcatgid = hexpensecategory.expCatgid ORDER BY expid DESC";
                        ...
                        if (mysqli_num_rows($resultExpense) > 0) {
                            while ($rowsExpense = mysqli_fetch_assoc($resultExpense)) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="exp_edit.php?expid=<?= $rowsExpense['expid']; ?>">
                                            Edit
                                        </a>
                                    </td>
                                </tr>
